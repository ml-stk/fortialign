...
function buildReport(items: MigrationFinding[], statistics: { configs: number; edits: number; commands: number; transformedCommands: number } | null, inv: ConfigurationInventory, graph: DependencyGraph | null) {
  ...
  return [
    ...
    '', '## Dependency assurance', `- Nodes: ${ds?.nodes ?? 0}`, `- Dependencies: ${ds?.edges ?? 0}`, `- Unresolved references: ${ds?.unresolved ?? 0}`, `- Orphan candidates: ${ds?.orphans ?? 0}`, `- Disabled referenced objects: ${ds?.disabledReferenced ?? 0}`, `- Consumers: ${ds?.consumers ?? 0}`,
    '', '### Unresolved by kind', ...Object.entries(db?.unresolvedByKind ?? {}).map(([key, value]) => `- ${key}: ${value}`),
    '', '### Unresolved by severity', ...Object.entries(db?.unresolvedBySeverity ?? {}).map(([key, value]) => `- ${key}: ${value}`),
    '', '### Orphan candidates by category', ...Object.entries(db?.orphanByCategory ?? {}).map(([key, value]) => `- ${key}: ${value}`),
    '', '### Unresolved diagnostics', ...(graph?.unresolved.length ? graph.unresolved.map(issue => `- ${issue.severity.toUpperCase()} / ${issue.kind}: ${issue.reference} — ${issue.path} — ${issue.reason}`) : ['- None']),
...
export default function App() {
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-200 p-6 flex flex-col gap-6">
    ...
    {graph && <section className="bg-slate-900 border border-slate-800 rounded-lg p-4"><div className="flex justify-between items-center mb-3"><h2 className="text-sm font-semibold uppercase tracking-wider">Dependency assurance</h2><span className="text-xs text-slate-500">Cross-object reference analysis</span></div><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">{[['Objects', ds?.nodes], ['Dependencies', ds?.edges], ['Unresolved', ds?.unresolved], ['Orphan candidates', ds?.orphans], ['Disabled refs', ds?.disabledReferenced], ['Consumers', ds?.consumers]].map(([label, value]) => <div key={String(label)} className="border border-slate-800 rounded p-3"><div className="text-xs text-slate-500">{label}</div><div className="text-lg font-semibold">{value}</div></div>)}</div><div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4"><div><h3 className="text-xs uppercase text-slate-500 mb-2">Unresolved by type</h3><div className="space-y-1">{Object.entries(db?.unresolvedByKind ?? {}).map(([key, value]) => <div key={key} className="flex justify-between text-sm"><span>{key}</span><span className="font-mono">{value}</span></div>)}</div></div><div><h3 className="text-xs uppercase text-slate-500 mb-2">Unresolved by severity</h3><div className="space-y-1">{Object.entries(db?.unresolvedBySeverity ?? {}).map(([key, value]) => <div key={key} className="flex justify-between text-sm"><span className={severityClass(key as MigrationFinding['severity'])}>{key}</span><span className="font-mono">{value}</span></div>)}</div></div><div><h3 className="text-xs uppercase text-slate-500 mb-2">Orphan candidates by category</h3><div className="space-y-1 max-h-32 overflow-auto">{Object.entries(db?.orphanByCategory ?? {}).map(([key, value]) => <div key={key} className="flex justify-between text-sm"><span>{key}</span><span className="font-mono">{value}</span></div>)}</div></div></div>{(graph.unresolved.length > 0 || graph.disabledReferenced.length > 0) && <div className="mt-4 space-y-2 max-h-72 overflow-auto">{graph.unresolved.map((issue, index) => <div key={`u-${index}`} className="border border-orange-900/50 rounded p-3 text-sm"><span className={severityClass(issue.severity as MigrationFinding['severity'])}>{issue.severity.toUpperCase()}</span> <span className="text-slate-400">{issue.kind}</span> <span className="font-mono text-slate-300">{issue.reference}</span><div className="text-slate-500">{issue.path} — {issue.reason}</div></div>)}{graph.disabledReferenced.map(node => <div key={`d-${node.id}`} className="border border-red-900/50 rounded p-3 text-sm"><span className="text-red-400 font-medium">DISABLED REFERENCED</span> <span className="font-mono text-slate-400">{node.name}</span><div className="text-slate-500">{node.path}</div></div>)}</div>}</section>}
