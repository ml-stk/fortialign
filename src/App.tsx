...
import { buildDependencyGraph, dependencySummary, type DependencyGraph } from './analysis/dependencyGraph';
...
const inventoryLabels: Array<[keyof ConfigurationInventory, string]> = [
  ['interfaces', 'Interfaces'], ['addressObjects', 'Address objects'], ['addressGroups', 'Address groups'], ['services', 'Services'], ['serviceGroups', 'Service groups'], ['firewallPolicies', 'Firewall policies'], ['virtualIps', 'VIPs'], ['ipPools', 'IP pools'], ['staticRoutes', 'Static routes'], ['sdwanMembers', 'SD-WAN members'], ['sdwanServices', 'SD-WAN rules'], ['sdwanHealthChecks', 'SD-WAN health checks'], ['ipsecPhase1', 'IPsec Phase 1'], ['ipsecPhase2', 'IPsec Phase 2'], ['userGroups', 'User groups'], ['authenticationServers', 'Authentication servers'], ['securityProfiles', 'Security profiles'], ['certificates', 'Certificates'], ['dhcpServers', 'DHCP servers'], ['management', 'Management objects'], ['otherConfigs', 'Other configuration entries']
];
...
function buildReport(items: MigrationFinding[], statistics: { configs: number; edits: number; commands: number; transformedCommands: number } | null, inv: ConfigurationInventory, graph: DependencyGraph | null) {
  ...
  return ['# FortiAlign V2 Migration & Assurance Report', '', `Source: ${fortigate100ETo120G714Profile.sourceModel} / ${fortigate100ETo120G714Profile.sourceFirmware}`, `Target: ${fortigate100ETo120G714Profile.destinationModel} / ${fortigate100ETo120G714Profile.destinationFirmware}`, '', '## Migration result', ...['BLOCK','MANUAL','REVIEW','TRANSFORMED','PASS'].map(status => `- ${status}: ${counts[status] ?? 0}`), '', '## Configuration inventory', ...inventoryLabels.map(([key, label]) => `- ${label}: ${totals[String(key)] ?? 0}`), '', '## Dependency assurance', `- Nodes: ${ds?.nodes ?? 0}`, `- Dependencies: ${ds?.edges ?? 0}`, `- Unresolved references: ${ds?.unresolved ?? 0}`, `- Orphan candidates: ${ds?.orphans ?? 0}`, `- Disabled referenced objects: ${ds?.disabledReferenced ?? 0}`, '', '## Migration statistics', `- Config blocks: ${statistics?.configs ?? 0}`, `- Edit blocks: ${statistics?.edits ?? 0}`, `- Commands: ${statistics?.commands ?? 0}`, `- Transformed commands: ${statistics?.transformedCommands ?? 0}`, '', '## Findings', ...items.map(item => `### ${item.id} — ${item.severity.toUpperCase()} — ${item.status}\n- **${item.title}**\n- ${item.message}\n- Path: ${item.sourcePath ?? 'n/a'}\n- Recommendation: ${item.recommendation}`), '', '> Generated by FortiAlign V2. Target-device validation remains mandatory before production deployment.'].join('\n');
}
...
export default function App() {
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-200 p-6 flex flex-col gap-6">
    ...
    {stats && <section className="grid grid-cols-2 lg:grid-cols-4 gap-3">{[['Config blocks', stats.configs], ['Edit blocks', stats.edits], ['Commands', stats.commands], ['Transformed', stats.transformedCommands]].map(([label, value]) => <div key={String(label)} className="bg-slate-900 border border-slate-800 rounded-lg p-4"><div className="text-xs text-slate-500 uppercase">{label}</div><div className="text-2xl font-bold">{value}</div></div>)}</section>}
    {inventory && <section className="bg-slate-900 border border-slate-800 rounded-lg p-4"><div className="flex justify-between items-center mb-3"><h2 className="text-sm font-semibold uppercase tracking-wider">Configuration inventory</h2><span className="text-xs text-slate-500">Pre-migration source inventory</span></div><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">{inventoryLabels.map(([key, label]) => <div key={String(key)} className="border border-slate-800 rounded p-3"><div className="text-xs text-slate-500">{label}</div><div className="text-lg font-semibold">{(inventory[key] as unknown[]).length}</div></div>)}</div></section>}
    {graph && <section className="bg-slate-900 border border-slate-800 rounded-lg p-4"><div className="flex justify-between items-center mb-3"><h2 className="text-sm font-semibold uppercase tracking-wider">Dependency assurance</h2><span className="text-xs text-slate-500">Cross-object reference analysis</span></div><div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">{[['Objects', ds?.nodes], ['Dependencies', ds?.edges], ['Unresolved', ds?.unresolved], ['Orphan candidates', ds?.orphans], ['Disabled refs', ds?.disabledReferenced], ['Consumers', ds?.consumers]].map(([label, value]) => <div key={String(label)} className="border border-slate-800 rounded p-3"><div className="text-xs text-slate-500">{label}</div><div className="text-lg font-semibold">{value}</div></div>)}</div>{(graph.unresolved.length > 0 || graph.disabledReferenced.length > 0) && <div className="mt-3 space-y-2 max-h-60 overflow-auto">{graph.unresolved.map((issue, index) => <div key={`u-${index}`} className="border border-orange-900/50 rounded p-3 text-sm"><span className="text-orange-400 font-medium">UNRESOLVED</span> <span className="font-mono text-slate-400">{issue.reference}</span><div className="text-slate-500">{issue.path} — {issue.reason}</div></div>)}{graph.disabledReferenced.map(node => <div key={`d-${node.id}`} className="border border-red-900/50 rounded p-3 text-sm"><span className="text-red-400 font-medium">DISABLED REFERENCED</span> <span className="font-mono text-slate-400">{node.name}</span><div className="text-slate-500">{node.path}</div></div>)}</div>}</section>}
    {findings.length > 0 && <section className="bg-slate-900 border border-slate-800 rounded-lg p-4"><div className="flex justify-between items-center mb-3"><h2 className="text-sm font-semibold uppercase tracking-wider">Migration findings</h2><span className="text-xs text-slate-500">{findings.length} findings</span></div><div className="max-h-80 overflow-auto space-y-2">{findings.map(item => <div key={item.id} className="border border-slate-800 rounded p-3 text-sm"><div className="flex gap-3"><span className="font-mono text-slate-500">{item.id}</span><span className={severityClass(item.severity)}>{item.severity.toUpperCase()}</span><span className="text-slate-400">{item.status}</span><strong>{item.title}</strong></div><p className="text-slate-400 mt-1">{item.message}</p><p className="text-slate-500 mt-1">{item.recommendation}</p></div>)}</div></section>}
