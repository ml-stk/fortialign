...
export default function App() {
  ...
  const buildReport = (items: MigrationFinding[], statistics: typeof stats, inv: ConfigurationInventory) => {
    ...
    return ['# FortiAlign Migration Report', '', `Source: ${fortigate100ETo120G714Profile.sourceModel} / ${fortigate100ETo120G714Profile.sourceFirmware}`, `Target: ${fortigate100ETo120G714Profile.destinationModel} / ${fortigate100ETo120G714Profile.destinationFirmware}`, '', '## Result', `- BLOCK: ${counts.BLOCK ?? 0}`, `- MANUAL: ${counts.MANUAL ?? 0}`, `- REVIEW: ${counts.REVIEW ?? 0}`, `- TRANSFORMED: ${counts.TRANSFORMED ?? 0}`, `- PASS: ${counts.PASS ?? 0}`, '', '## Configuration inventory', ...inventoryLabels.map(([key, label]) => `- ${label}: ${totals[String(key)] ?? 0}`), '', '## Statistics', `- Config blocks: ${statistics?.configs ?? 0}`, `- Edit blocks: ${statistics?.edits ?? 0}`, `- Commands: ${statistics?.commands ?? 0}`, `- Transformed commands: ${statistics?.transformedCommands ?? 0}`, '', '## Findings', ...items.map(item => `### ${item.id} — ${item.severity.toUpperCase()} — ${item.status}\n- **${item.title}**\n- ${item.message}\n- Path: ${item.sourcePath ?? 'n/a'}\n- Recommendation: ${item.recommendation}`), '', '> Generated by FortiAlign. Target-device validation is required before production deployment.'].join('\n');
  };
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setInventory(inv); setMigratedText(outputText); setFindings(validation); setStats(result.statistics);
    if (downloadUrl) URL.revokeObjectURL(downloadUrl); if (reportUrl) URL.revokeObjectURL(reportUrl);
    setDownloadUrl(URL.createObjectURL(new Blob([outputText], { type: 'text/plain' })));
    setReportUrl(URL.createObjectURL(new Blob([buildReport(validation, result.statistics, inv)], { type: 'text/markdown' })));
  };

  return <div className="min-h-screen bg-slate-950 text-slate-200 p-6 flex flex-col gap-6">
    <header className="flex flex-col lg:flex-row justify-between items-center bg-slate-900 p-4 rounded-lg border border-slate-800 gap-4"><div><h1 className="text-xl font-bold text-slate-100">FortiAlign — FortiGate Migration & Assurance</h1><p className="text-sm text-slate-400">100E / FortiOS 7.0.19 → 120G / FortiOS 7.4.12</p></div><div className="flex gap-3 items-center flex-wrap justify-center"><label className="cursor-pointer px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded font-medium text-sm">Import .conf<input type="file" accept=".conf,.txt" className="hidden" onChange={handleFileUpload} /></label>{downloadUrl && <a href={downloadUrl} download="fortialign-migrated.conf" className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded font-medium text-sm">Download Config</a>}{reportUrl && <a href={reportUrl} download="fortialign-migration-report.md" className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded font-medium text-sm">Download Report</a>}</div></header>
